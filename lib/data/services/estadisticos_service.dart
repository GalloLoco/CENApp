...
class EstadisticosService {
  ...
  static Map<String, dynamic> calcularEstadisticas(List<dynamic> valores) {
    ...
    if (valores.first is num) {
      ...
      // Cálculo del rango
      double rango = (valoresNumericos.last - valoresNumericos.first).toDouble();
      
      // Cálculo de la varianza
      double sumCuadrados = valoresNumericos.fold(
        0.0, 
        (prev, curr) => prev + pow(curr - media, 2)
      );
  ...
  static Map<String, dynamic> analizarUsoViviendaTopografia(List<FormatoEvaluacion> formatos) {
  ...
  static Map<String, List<dynamic>> _extraerUsosVivienda(List<FormatoEvaluacion> formatos) {
    ...
    for (var formato in formatos) {
      ...
      for (var categoria in categoriasUso) {
        if (usosFormato.containsKey(categoria) && usosFormato[categoria] == true) {
          usos[categoria]!.add(1); // Agregamos 1 para poder calcular estadísticas numéricas
        }
      }
  ...
  static Map<String, List<dynamic>> _extraerTopografia(List<FormatoEvaluacion> formatos) {
    ...
    for (var formato in formatos) {
      ...
      Map<String, bool> topografiaFormato = formato.informacionGeneral.topografia;
      
      // Llenar listas por categoría
      for (var categoria in categoriasTopografia) {
        if (topografiaFormato.containsKey(categoria) && topografiaFormato[categoria] == true) {
          topografia[categoria]!.add(1); // Agregamos 1 para poder calcular estadísticas numéricas
        }
      }
    }
